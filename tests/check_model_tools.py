...
def main() -> None:
    ...
    require(backend, "Remote model call failed. AgentLens did not hide the failure with a local fallback answer.", "configured remote model should not silently fallback")
    require(backend, "127.0.0.1/localhost refers to the machine running the AgentLens backend", "localhost endpoint hint")
    require(chat_router, "from .extensions import agent_chat", "chat endpoint can route automatic tool requests to agent handler")
    require(backend, "LangGraphAgentEngine", "LangGraph agent runtime")
