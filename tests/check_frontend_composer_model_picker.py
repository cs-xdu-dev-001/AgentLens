...
def main() -> None:
    ...
    require(component, 'aria-keyshortcuts={"Alt+R"}', "reasoning shortcut semantics")
    require(component, 'pickerFocusRef.current = shortcut === "r" ? "reasoning" : "model"', "shortcut focus routing")
    require(component, 'data-reasoning-index={index}', "reasoning roving focus index")
    require(component, 'tabIndex={item.id === reasoningEffort ? 0 : -1}', "reasoning roving tab stop")
    require(component, '"ArrowLeft", "ArrowUp", "ArrowRight", "ArrowDown", "Home", "End"', "reasoning keyboard navigation")
    require(component, '.composer-context-section button, .composer-model-manage', "non-model control keyboard guard")
