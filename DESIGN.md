...
AgentLens不是仪表盘陈列馆，也不是重新包装的聊天框。它是一张安静、专业的Agent工作台：主体让用户专注任务，右侧运行面板把模型、工具、Skills、MCP、记忆和计划的关键状态讲清楚。界面保持低饱和黑白中性色，不用装饰制造“高级感”，而通过清晰层级、稳定密度和真实状态建立可信度。
...
Linux CLI的首选展示层使用React/Ink，Python/LangGraph仍是唯一执行与权限边界。两层只通过有版本的JSONL事件交互；Ink不可用时回退Textual。命令建议贴近输入区，运行状态原位更新，审批与权限选择保持键盘优先，不使用占满终端的管理弹窗。
...
侧栏导航与任务标题使用14px正文、36px基础行高，默认透明、悬停柔和灰面、选中态提高对比并保持当前位置可辨识。图标只辅助识别，不替代文字。移动端将主导航收进抽屉，但新对话和当前任务入口保持可达。

侧栏样式统一由`sidebar-polish.css`收口，不在`template.css`尾部追加侧栏补丁。桌面侧栏272px、折叠轨道64px；功能入口保持单列，不随窗口高度切换成网格。历史标题优先，已完成和已取消的任务不重复显示进度摘要；运行中和需要处理的任务保留状态与进度。账户菜单复用Radix，支持方向键、Esc和焦点返回。

### Tooltips

图标操作复用`Tooltip`组件与全局`TooltipProvider`，由Radix处理聚焦、悬停、Esc和边缘避让。触发器使用`asChild`保留原按钮、事件和布局，不额外包装DOM；可见提示不能替代`aria-label`，同一入口不再叠加原生`title`。提示只放操作名称、必要说明或真实快捷键，不放交互控件。菜单打开后隐藏其入口提示，触屏点击仍直接执行原操作；减少动态效果时直接显示提示。

### Composer

输入框是聊天页的主要操作面。文本输入占据主层级；附件、模型和Skill选择以同一行的紧凑入口出现；发送按钮保持明确但不过度放大。两行结构必须视觉相连，不能像两个独立工具条。

聊天空态class由`ChatPage`根据消息组件上报的状态统一渲染，不手动修改React持有的class，避免跨页面返回时丢失欢迎区和输入框布局。

窄屏空态顶栏仍保留当前任务标题；操作入口可以压缩或省略，但不能让工作上下文在移动端消失。

### Agent Run Trace

运行轨迹以状态和任务语义为第一层：名称、状态、耗时必须一眼可读。输入、输出、错误、来源和诊断信息在选中节点后展开。不得渲染隐藏思维链、Secret、Token或未经脱敏的内部日志。

运行中的ThinkingOrb复用`thinking-orbs`并跟随AgentLens的`mono-light/mono-dark`主题，避免状态动画与工作台反差。

运行连接中断时保留当前任务和恢复入口；网络恢复或页面回到前台后只自动尝试一次已有resume操作，失败仍显示手动恢复，不重复启动任务。

工具输出沿用工作台浅深色主题，正文与命令使用14px等宽字体。短结果按内容收缩，长结果在有界区域内滚动；可选命令行不能改变正文与底部元数据的布局角色。等待输出、无输出、标准错误和非零退出须明确区分，跟随与复制操作始终保留。
...
- **Don't** 混用AgentLens与旧产品名称，也不要重排导航或改变核心交互，除非用户另行确认。
