...
The backend routes Agent requests through an internal execution-engine interface. `KNOWFLOW_AGENT_ENGINE=current` remains the default and preserves the legacy tools, MCP, Skills, approvals, task planning, and memory workflows. `KNOWFLOW_AGENT_ENGINE=langgraph` enables the incremental `START -> model -> tools -> model -> END` graph while reusing the selected model configuration, Chat Completions or Responses API transport, streaming events, tool audits, and run records. It supports per-user `web_search` plus enabled MCP read and write tools. Read-only MCP calls run directly; write, destructive, or unknown-risk calls use LangGraph `interrupt()` and a durable user/run/tool-call approval record before the remote handler can run. Approval resumes the same checkpoint. Each approved write is atomically claimed and its result is persisted; if the process stops after the remote side effect but before its result is recorded, KnowFlow reports an indeterminate execution instead of automatically repeating the write. Tool schemas and execution use the same engine allow-list, so hallucinated or non-migrated names cannot reach a handler. Checkpoints are keyed by user and run ID in the separate SQLite file. Skills and task-planner nodes remain on `current`; Mem0 recall and background write still run outside the LangGraph state graph. Empty or unknown engine values safely fall back to `current`.
